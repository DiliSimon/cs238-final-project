...
class Network:
    ...
    def send(self):
        ...
        self.sock.sendall(bytes(self.sendBuffer.read(), 'utf-8'))
    ...
    def getInt(self):
        ...
        return struct.unpack("!i",self.recvBuffer.buffer)[0]

    def getDouble(self):
        s = self.recvBuffer.read(kDoubleSize)
        return struct.unpack("!d",self.recvBuffer.buffer)[0]
    ...
    def getAbstractType_list(self):
        ...
        if numInts > 0:
            ...
            returnStruct.intArray = list(struct.unpack("!%di" % numInts, self.recvBuffer.buffer))
        if numDoubles > 0:
            s = self.recvBuffer.read(numDoubles*kDoubleSize)
            returnStruct.doubleArray = list(struct.unpack("!%dd" % numDoubles, self.recvBuffer.buffer))
        if numChars > 0:
            s = self.recvBuffer.read(numChars*kCharSize)
            returnStruct.charArray = list(struct.unpack("!%dc" % numChars, self.recvBuffer.buffer))
        return returnStruct
    ...
    def putInt(self,value):
        ...
        self.sendBuffer.write(str(value))
