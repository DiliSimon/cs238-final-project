...
class InvasiveEnvironment(Environment):
    ...
    def __init__(self, simulationParameterObj=None, actionParameterObj=None, Bad_Action_Penalty=-10000,
        ...
        if simulationParameterObj != None:
            ...
            reachArrivalRates = array([[random.randint(100, 1000) for i in range(2)] for i in range(nbrReaches)])
            reachArrivalProbs = array([[random.random() for i in range(2)] for i in range(nbrReaches)])
    ...
    def setRandomState(self):
        S = array([random.randint(1, 3) for i in
                   range(self.simulationParameterObj.nbrReaches * self.simulationParameterObj.habitatSize)])
